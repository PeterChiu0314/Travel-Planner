  ...
  itinerary_data as (
    select coalesce(
      jsonb_agg(
        jsonb_build_object(
          'id', i.id,
          'date', (trip_data.start_date + i.day_index),
        ...
        order by i.day_index, i.start_time, i.sort_order
      ),
      '[]'::jsonb
    ) as items
    from public.itinerary_items i
    join trip_data on trip_data.id = i.trip_id
    where i.day_index >= 0
      and i.day_index < ((trip_data.end_date - trip_data.start_date) + 1)
  ),
