create or replace function app_private.reorder_itinerary_destination_packages(
  target_trip_id uuid,
...
begin
  ...
  if item_updated_at_baselines is null
    ...
    or jsonb_object_length(item_updated_at_baselines)
...
revoke execute on function app_private.reorder_itinerary_destination_packages(uuid, integer, uuid[], uuid[], jsonb) from public;
revoke execute on function app_private.reorder_itinerary_destination_packages(uuid, integer, uuid[], uuid[], jsonb) from anon;
...
create or replace function public.reorder_itinerary_destination_packages(
  target_trip_id uuid,
  target_day_index integer,
  slot_item_ids uuid[],
  package_source_item_ids uuid[],
...
set search_path = public, app_private
as $$
  select app_private.reorder_itinerary_destination_packages(
    target_trip_id,
    target_day_index,
    slot_item_ids,
...
revoke execute on function public.reorder_itinerary_destination_packages(uuid, integer, uuid[], uuid[], jsonb) from public;
revoke execute on function public.reorder_itinerary_destination_packages(uuid, integer, uuid[], uuid[], jsonb) from anon;
revoke execute on function public.reorder_itinerary_destination_packages(uuid, integer, uuid[], uuid[], jsonb) from authenticated;
grant execute on function public.reorder_itinerary_destination_packages(uuid, integer, uuid[], uuid[], jsonb) to authenticated;

comment on function public.reorder_itinerary_destination_packages(uuid, integer, uuid[], uuid[], jsonb) is
  'Atomically permutes timed-visit destination packages across fixed slots and preserves only still-adjacent directed transportation cards.';
