...
test("visit editor stages alternative changes and saves them with the main itinerary", async ({ page }) => {
  ...
  await expect(visit.locator(".item-expanded-alternative")).toHaveCount(0);
  ...
  await expect(form.locator(".visit-alternative-create-summary")).toHaveCSS("height", "28px");
  await expect(form.locator(".visit-alternative-create-summary .lucide-chevron-right")).toHaveCount(1);
  ...
  await expect(form.getByRole("button", { name: "刪除備案" })).toHaveCount(0);
  ...
  await expect(form.locator(".visit-alternative-summary .lucide-chevron-right")).toHaveCount(1);
  await expect(form.getByRole("button", { name: "刪除備案" })).toHaveCount(0);
  ...
  const budgetList = expandedDetails.locator(".linked-budget-list");
  const budgetHeading = budgetList.locator(".item-expanded-budget-heading");
  const budgetTitle = budgetHeading.locator("strong");
  const budgetContent = budgetList.locator(".item-expanded-budget-content");
  const emptyBudgetTag = budgetContent.locator(".muted-text");
  const alternativeBlock = expandedDetails.locator(".item-expanded-alternative");
  const alternativeHeading = alternativeBlock.locator(".item-expanded-alternative-heading");
  const alternativeContent = alternativeBlock.locator(".item-expanded-alternative-content");
  ...
  await expect(budgetContent.locator(".item-expanded-section-divider")).toHaveCount(1);
  await expect(budgetList).toHaveCSS("margin-top", "8px");
  await expect(emptyBudgetTag).toHaveText("尚未連動預算");
  await expect(emptyBudgetTag).toHaveClass(/pill/);
  await expect(emptyBudgetTag).toHaveCSS("font-size", "12px");
  await expect(emptyBudgetTag).toHaveCSS("font-weight", "500");
  await expect(alternativeHeading).toContainText("備案");
  await expect(alternativeHeading).toHaveCSS("font-size", "14px");
  await expect(alternativeHeading).toHaveCSS("font-weight", "400");
  await expect(alternativeHeading.locator(".lucide-files")).toHaveCount(1);
  await expect(alternativeHeading.locator(".lucide-files")).toHaveCSS("width", "16px");
  await expect(alternativeBlock.locator(".item-expanded-section-divider")).toHaveCount(1);
  await expect(alternativeSummary).toContainText("交通・備案 F");
  await expect(alternativeSummary).toHaveCSS("height", "24px");
  await expect(alternativeSummary).toHaveCSS("font-weight", "400");
  ...
  await expect(expandedMapLink).toHaveCSS("padding-left", "6px");
  await expect(expandedMapLink).toHaveCSS("padding-right", "6px");
  await expect(expandedMapLink).toHaveCSS("margin-left", "-6px");
  await expect(expandedMapLink).toHaveCSS("margin-top", "12px");
  ...
  const budgetHeadingBox = await budgetHeading.boundingBox();
  const budgetContentBox = await budgetContent.boundingBox();
  const budgetTagBox = await emptyBudgetTag.boundingBox();
  const alternativeBlockBox = await alternativeBlock.boundingBox();
  const alternativeHeadingBox = await alternativeHeading.boundingBox();
  const alternativeBox = await alternativeSummary.boundingBox();
  const flipBox = await visit.locator(".alternative-flip-button").boundingBox();
  const mapLinkBox = await expandedMapLink.boundingBox();
  await expect(expandedDetails).toHaveCSS("padding-left", "30px");
  await expect(expandedDetails).toHaveCSS("padding-right", "30px");
  expect(budgetTagBox.width).toBeLessThan(expandedBox.width - 30 - 30);
  expect(budgetContentBox.x).toBeGreaterThan(budgetHeadingBox.x + budgetHeadingBox.width);
  expect(Math.abs(budgetContentBox.y - budgetHeadingBox.y)).toBeLessThanOrEqual(5);
  expect(alternativeBlockBox.width).toBeCloseTo(expandedBox.width - 30 - 30, 0);
  expect(alternativeBox.x).toBeGreaterThan(alternativeHeadingBox.x + alternativeHeadingBox.width);
  expect(Math.abs(alternativeBox.y - alternativeHeadingBox.y)).toBeLessThanOrEqual(5);
  expect(alternativeBox.x + alternativeBox.width).toBeLessThanOrEqual(flipBox.x - 4);
  expect(mapLinkBox.y).toBeGreaterThan(alternativeBox.y + alternativeBox.height);
  expect(Math.abs(mapLinkBox.x + 6 - itemDetailsBox.x)).toBeLessThanOrEqual(1);
  await budgetList.evaluate((element) => { element.style.width = "180px"; });
  const wrappedBudgetHeadingBox = await budgetHeading.boundingBox();
  const wrappedBudgetContentBox = await budgetContent.boundingBox();
  expect(wrappedBudgetContentBox.y).toBeGreaterThan(wrappedBudgetHeadingBox.y);
  await budgetList.evaluate((element) => { element.style.width = ""; });
  await alternativeBlock.evaluate((element) => { element.style.width = "120px"; });
  const wrappedAlternativeHeadingBox = await alternativeHeading.boundingBox();
  const wrappedAlternativeContentBox = await alternativeContent.boundingBox();
  expect(wrappedAlternativeContentBox.y).toBeGreaterThan(wrappedAlternativeHeadingBox.y);
  await alternativeBlock.evaluate((element) => { element.style.width = ""; });
  await visit.getByTitle("編輯").click();
  form = page.locator(".timeline-day-column.active .item-form:not(.transport-editor-form)");
  await form.getByRole("button", { name: "更多設定" }).click();
  await form.locator(".visit-alternative-summary").click();
  await expect(form.locator(".form-mode-label")).toHaveText("編輯備案");
  const editorDeleteButton = form.getByRole("button", { name: "刪除備案" });
  ...
  await expect(alternativeDeleteDialog).toBeVisible();
  await alternativeDeleteDialog.getByRole("button", { name: "刪除備案" }).click();
  await expect(form.locator(".form-mode-label")).toHaveText("編輯行程");
  await form.getByRole("button", { name: "更多設定" }).click();
...
test("transport editor uses compact floating fields and preserves exact minute input", async ({ page }) => {
  ...
  const categoryTrigger = categoryField.getByRole("combobox", { name: "交通類別" });
  ...
  await expect(categoryField.locator("select")).toHaveCount(0);
  await expect(categoryTrigger.locator(".timeline-time-menu-toggle")).toHaveCSS("width", "24px");
  await expect(categoryTrigger.locator(".lucide-chevron-down")).toHaveCount(1);
  await categoryTrigger.click();
  const categoryMenu = categoryField.getByRole("listbox", { name: "交通類別選項" });
  ...
  await expect(form.locator(".transport-editor-note-field")).toHaveCSS("height", "62px");
  expect((await note.boundingBox()).height).toBeLessThanOrEqual(60);
  const noteScrollbar = await note.evaluate((textarea) => ({
    buttonDisplay: getComputedStyle(textarea, "::-webkit-scrollbar-button").display,
    thumbBackground: getComputedStyle(textarea, "::-webkit-scrollbar-thumb").backgroundColor,
    width: getComputedStyle(textarea, "::-webkit-scrollbar").width,
  }));
