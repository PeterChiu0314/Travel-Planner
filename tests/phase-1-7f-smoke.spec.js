...
test("visit editor stages alternative changes and saves them with the main itinerary", async ({ page }) => {
  ...
  await expect(budgetTitle).toHaveCSS("font-weight", "600");
  await expect(budgetTitle).toHaveCSS("padding-left", "8px");
  ...
  expect(budgetTagBox.width).toBeCloseTo(expandedBox.width - 30 - 30, 0);
  expect(Math.abs(alternativeBlockBox.width - budgetTagBox.width)).toBeLessThanOrEqual(1);
