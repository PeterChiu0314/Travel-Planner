...
test("Phase 5.8b keeps the Map behind a full-height Dayboard glass panel", () => {
  ...
  expect(stylesSource).toContain("--timeline-dayboard-glass-background: var(--map-glass-bg-fallback)");
  expect(stylesSource).toContain("--timeline-dayboard-glass-background: var(--map-glass-bg)");
  expect(stylesSource).toContain("--timeline-dayboard-glass-border: 1px solid var(--map-glass-border)");
  expect(stylesSource).toContain("--map-glass-bg: rgba(255, 255, 255, 0.4)");
  expect(stylesSource).toContain("--map-glass-border: rgba(255, 255, 255, 0.55)");
  expect(stylesSource).toContain("-webkit-backdrop-filter: blur(var(--map-glass-blur)) saturate(var(--map-glass-saturation))");
  ...
  expect(stylesSource).toContain("background: rgba(255, 255, 255, 0.85)");
  expect(stylesSource).toContain(".timeline-item");
  expect(stylesSource).toContain(".transport-card");
  expect(stylesSource).toContain(".timeline-workbench .item-form");
...
test("Phase 5.8 keeps Timeline card gaps compact and uses library transport icons", () => {
  ...
  expect(stylesSource).toContain("grid-template-columns: 13px max-content minmax(24px, 1fr)");
  expect(stylesSource).toContain("grid-template-columns: 13px max-content minmax(24px, 1fr);\n  gap: 2px");
...
test("Phase 5.9 visit editor uses compact linked time controls and a draft-only point workflow", () => {
  ...
  expect(appSource).toContain('source: isMapUrlPoint || !isMapSearchReplaceActive ? "places-details" : "places-replace"');
  expect(googleProviderSource).toContain(': "更改地點"');
...
test("Phase 5.7b-1 Google provider exposes route edit mode skeleton only in Google map", () => {
  ...
  expect(googleProviderSource).toContain('isMapSearchReplaceActive ? "取消搜尋替換" : isPickingMapPoint ? "取消選點" : "離開路線編輯模式"');
  expect(googleProviderSource).toContain("isMapSearchReplaceActive ? exitMapSearchReplace : isPickingMapPoint ? onCancelMapPointPick : exitRouteEditMode");
  ...
  expect(googleProviderSource).toContain("disabled={showPlacesSearchCancel ? false : isRouteEditMode || isMapSearchReplaceActive}");
  expect(googleProviderSource).toContain('title={showPlacesSearchCancel ? "清除搜尋" : isPickingMapPoint ? "取消選點" : "在地圖選點新增景點"}');
  expect(googleProviderSource).toContain('aria-label={showPlacesSearchCancel ? "清除搜尋" : isPickingMapPoint ? "取消選點" : "在地圖選點新增景點"}');
  expect(googleProviderSource).toContain("if (!isPickingMapPoint && !isRouteEditMode) onFocusItem?.(marker.itemId)");
...
test("Phase 5.5 Google map area custom point add flow stays provider scoped", () => {
  ...
  expect(googleProviderSource).toContain('onStartMapPointPick?.(isMapAddLocationActive ? "map-add" : hasActiveMapPointEditor ? "editor" : "map-add")');
...
test("Phase 5.8a uses a compact custom numbered destination marker", () => {
  ...
  expect(googleProviderSource).toContain("timelineTypeMarkerTextColor");
