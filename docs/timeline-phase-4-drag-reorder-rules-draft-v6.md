...
Date: 2026-06-23
Status: Draft for discussion / updated with Phase 4.5b Transportation Role Model

> 本文件整理目前討論過的 Timeline Phase 4 後續規則。
> 目前專案已完成至 Phase 4.5 Hotfix 3；本次補上 Phase 4.5b Transportation Role Model，重新定義一般交通、尾端待配對交通、尾端形成交通 pair 與 untimed 的互動規則。
...
Phase 4.5b：Transportation Role Model（一般交通 / 尾端交通 / 尾端形成交通 pair）
Phase 4.5c：Phase 4.5 後續穩定化（保留）
...
- 先讓 transportation role model 穩定，避免 tail transport 與 normal transport 在 untimed 情境下互相誤判。
...
#### 1. 有 normal_pair 的 pair，中間不可插入 untimed visit
...
#### 3. 主動 untimed 插入已成立交通 pair 時直接拒絕

untimed visit 不提供 Phase 4.3 的 Restore / Delete flow。

只要目標位置在已有 `normal_pair` 或已成立 `tail_promoted_pair` 的中間，就直接禁止插入。
...
- 不新增交通卡
- 不影響 timed visits 時間
- 不可插入已有 normal_pair / tail_promoted_pair 的中間
- 若該 untimed 本身已有相關交通卡，拖曳前需提示並依確認結果移除受影響交通卡
...
- 若 4.5b 完成後仍有 untimed / transportation 邊界問題，另開 4.5c 處理。
...
4.5b 先建立 Transportation Role Model，正式分清 normal_pair / tail_pending / tail_promoted_pair，讓一般交通與尾端交通套用不同 untimed 規則：一般交通 endpoint 變 untimed 時保留 warning；尾端交通遇到 untimed 不形成 pair，只有 untimed 設定合理時間後才升級為 tail_promoted_pair；tail_promoted_pair 的 next endpoint 再變 untimed 時退回 tail_pending；
4.5c 保留給後續 4.5 穩定化；
