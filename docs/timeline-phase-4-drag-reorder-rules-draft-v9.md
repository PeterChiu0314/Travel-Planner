# Timeline Phase 4 拖曳重排 / 多人協作 / 固定卡 / 未設時間規則草稿 v9

Date: 2026-06-29
Status: Full rules draft for next chat / updated after Phase 4.5b + Phase 4.5c implementation

> 本文件以 v6 為底完整保留原有規則，只補上 Phase 4.5b / 4.5c 已完成後的最新狀態與修正。
> 本版修回原本 timed / untimed 定義：只有 `start_time` 與 `end_time` 都存在才是 timed visit；任一缺少即視為 untimed / partial time，不可參與 Phase 4.6 duration-based auto-continuation。
> 本版也補上 Phase 4.5c 的 mixed visual drag target、`brokenTransportIds`、以及 timed / untimed 插入既有交通 pair 中間時的確認刪除規則。
...
- Phase 4.5 / 4.5b / 4.5c 已先把 untimed mixed order 與 transportation role model 穩定，讓 Phase 4.6 可以專注在 timed visit drag auto-continuation。
- Phase 4.6 只處理無 fixed anchor 或不跨 fixed anchor 的基本 timed drag auto-continuation。
- Phase 4.6 必須保留每張 complete timed visit 自己的 duration，不可交換時間 slot。
- Phase 4.7 再處理固定卡作為時間錨點的進階拖曳規則。
...
4.6 再做 complete timed visit 拖曳後自動調整時間，重點是保留每張卡自己的原本停留時長，不可交換時間 slot；partial time / start-only / end-only 均不可參與 duration-based continuation；
