# Timeline Phase 4 拖曳重排 / 多人協作 / 固定卡 / 未設時間規則草稿 v12

Date: 2026-06-30
Status: Full rules draft for next chat / updated after Phase 4.7 completion + Phase 4.8a dnd-kit local sortable preview + Phase 4.8b/4.8c split

> 本文件以 v11 為底更新 Phase 4.7 已完成後的最新規則。
> Phase 4.7 已完成 fixed anchor continuation segments：fixed anchor 將同一天切成可調整區段；拖曳跨固定卡後，重算受影響 fixed 區段內的非固定 complete timed visits；若撞到下一個 fixed anchor，從第一張塞不下的非固定 timed visit 開始轉為 untimed，並保留 after-drop mixed visual order。
> Phase 4.7a 已修正 fixed-adjacent gap 被 early no-op 判斷擋掉的問題；Phase 4.7b 已修正 fixed overflow conversion 後既有 untimed 必須依 after-drop mixed visual order 一起 rebase，避免跳到 fixed anchor 下方或舊 slot。
> Phase 4.8 重新拆分為：4.8a dnd-kit Local Sortable Drag Preview、4.8b Demo Timeline Data Parity Polish、4.8c Collaborative Drag Presence。
> Phase 4.8a 已導入 dnd-kit Sortable / DragOverlay 做本機拖曳視覺預覽；此預覽只屬 local UI，不寫 DB、不呼叫 reorder RPC、不改時間、不刪或新增交通卡，drop 後仍走既有 Phase 4.7 正式 reorder flow。
> 本版仍保留 Phase 4.5b / 4.5c transportation role model、mixed visual drag target、`brokenTransportIds`，以及 timed / untimed 插入既有交通 pair 中間時的確認刪除規則。
...
Phase 4.8c：Collaborative Drag Presence，多人拖曳中狀態提示
...
- 不把拖曳、自動時間、固定卡、未設時間卡、交通卡限制、拖曳動畫與多人 presence 全部塞進同一個 Phase。
...
- Phase 4.7 已完成 fixed anchor continuation segments：fixed anchor 切分 day segments，受影響區段內依 Phase 4.6 規則接續，撞到下一個 fixed anchor 時從第一張塞不下的非固定 timed visit 開始轉 untimed。
- Phase 4.7a / 4.7b 已收斂 fixed-adjacent gap 與 overflow untimed visual order hotfix。
- Phase 4.8a 已完成本機 dnd-kit Sortable / DragOverlay 拖曳視覺預覽；它只處理 local UI，不混入正式 reorder RPC。
- 因使用者本機 `run dev` 主要依賴 `/demo/timeline` 測試 UI，下一步應先處理 Phase 4.8b Demo Timeline Data Parity Polish，避免 Demo 資料模式誤導 Formal 行為判斷。
- 多人拖曳中的 presence 只做暫時視覺狀態與同日保守鎖定，應延後到 Phase 4.8c，不混入 4.8a/4.8b。
- Phase 4.9 再進 Map 整合前置設計；Phase 4.10 做 QA 與交接。
...
---
## 11. Phase 4.8：Sortable Drag Preview / Demo Parity / Collaborative Drag Presence

> Phase 4.8 已重新拆分。
> 4.8a 已完成本機 dnd-kit Sortable Drag Preview。
> 4.8b 建議先處理 Demo Timeline Data Parity Polish。
> 4.8c 再處理 Collaborative Drag Presence。
...
- 寫入 `itinerary_items`
- 呼叫 reorder RPC
- 更新 `start_time / end_time`
- 轉 untimed
- 刪除 transportation cards
- 新增 transportation cards
- 清除 draft
- 釋放 edit lock
- 移動 alternatives / linked budgets 的正式資料
...
```text
local dnd-kit drag end
→ commitVisitDrop()
→ Phase 4.7 timed reorder flow 或 existing untimed reorder flow
→ 若需要，顯示 brokenTransportIds confirmation
→ confirm 後呼叫 Formal RPC / Demo planner
→ 成功後顯示正式結果
```

#### Cancel / invalid / cleanup

以下情境必須清除 local preview state，並回到 authoritative order：

- drop cancel
- invalid target
- Esc
- pointerup / dragend without valid target
- component unmount
- day switch
- route switch
- RPC fail / reload
...
- 不做多人 presence。
- 不同步 ghost card 給其他成員。
- 不讓其他使用者畫面跟著重排。
- 不改 Demo 為另一套假成功展示模式。
- 不重寫 Phase 4.7 planner / RPC。
- 不修改 Supabase migration。

---

### 11.2 Phase 4.8b：Demo Timeline Data Parity Polish（建議下一步）
...
使用者本機 `npm run dev` 時主要依賴 `/demo/timeline` 查看 UI。

若 Demo 資料模式與 Formal 不一致，會導致：

- 交通卡 pair 行為與正式頁不同。
- transport visual attachment 在 Demo 中卡位或顯示錯誤。
- dnd-kit preview QA 誤判。
- 之後 4.8c presence UI 也會在 Demo 上被錯誤資料形狀干擾。

因此 4.8b 應先讓 Demo 更像 Formal 的本地模擬版。

#### 4.8b 範圍

4.8b 可處理：

- 補齊 Demo mock transportation 的 `transport_role / from_item_id / to_item_id`。
- 確認 Demo flow entries 與 Formal 使用同一個 `ItineraryTimeline` / CSS / dnd-kit render path。
- 確認 Demo local state 更新後能模擬 Formal RPC 成功後的結果。
- 移除或修正 Demo 專屬的簡化 transportation render path。
- 補 Demo-only regression / browser QA，避免 Demo 卡在舊資料模型。
...
### 11.3 Phase 4.8c：Collaborative Drag Presence（後續）

#### 核心概念

當某位成員正在拖曳某一天的 timeline visit / destination card 時，其他成員可以看到拖曳狀態，但不能同時拖曳該日行程。
...
- timed visit drag
- untimed visit drag
- mixed visual order 會改變的 destination card drag
...
- 插入線不顯示文字。
- 不讓其他使用者畫面中的卡片真的重排。
- 不同步 local ghost card / DragOverlay。
- 不顯示大型 modal / toast / error banner。
- 不讓其他使用者看到拖曳者的完整 dnd-kit local preview order。

#### Presence 防卡死規則

Collaborative Drag Presence 必須避免 stale drag 卡死整天。

presence payload 建議包含：
...
- 拖曳中每 3～5 秒 heartbeat 更新 `lastSeenAt`。
- foreign presence 超過 10～15 秒未更新即視為 stale，不得繼續阻擋拖曳。
- 單次 drag 最長 60～90 秒；超過需自動清除或取消 presence。
- drop success / cancel / invalid drop / RPC success / RPC fail / unmount / day switch / route switch / logout 都必須 cleanup。
- 本機若發現自己的 presence 存在，但本機其實沒有正在拖曳，應自動清除或覆蓋。
...
- 已新增並套用 022 migration；後續不可修改已套用 022
- 既有測試資料可清理或重建
- Demo mock data 同步更新
...
dnd-kit Local Sortable Drag Preview。已完成。

範圍：

- 使用 dnd-kit Sortable / DragOverlay 做本機拖曳視覺預覽。
- Sortable item 只包含 destination visit ids。
- transportation card 不可拖曳、不加入 sortable items，但可作為前一張 destination wrapper 的 visual attachment 跟著 local preview movement。
- 拖曳中不寫 DB、不改時間、不轉 untimed、不刪或新增交通卡、不呼叫 RPC。
- drop 後仍走既有 Phase 4.7 reorder flow。
- Formal / Demo 共用 ItineraryTimeline 與 CSS。

### Phase 4.8b

Demo Timeline Data Parity Polish。建議下一步。

範圍：

- 讓 Demo mock data shape / transport role / pair 欄位 / render flow 更接近 Formal。
- Demo 仍不接 Supabase / Auth / Realtime / Draft / Edit Lock。
- Demo local state 模擬 Formal RPC 成功後結果。
- 目標是讓本機 `/demo/timeline` UI QA 不被 Demo 資料模式誤導。
...
- 同一天只允許一位成員拖曳 timeline visit / destination card。
- 其他成員看到「{userName} 正在拖曳」與淡淡插入線。
- 其他成員該日拖曳暫時 disabled。
- 不同步本機 ghost card / DragOverlay。
- presence 需有 heartbeat、timeout、max duration 與 cleanup，避免卡死。
...
5. Phase 4.8b 是否要補齊 Demo mock data 的所有 transport_role / from_item_id / to_item_id 欄位，並清除與 Formal 不一致的簡化 render path。
6. Phase 4.8b 完成後，是否需要把 Demo parity 規則同步整理進 CURRENT_TASK closeout handoff。
7. Phase 4.8c Collaborative Drag Presence 的技術實作要使用 Supabase Realtime presence、獨立 presence table，或既有 Realtime channel metadata，待實作前確認。
8. Phase 4.8c presence heartbeat / timeout / max duration / tab close / drag cancel / RPC fail 後的 cleanup 策略，需在實作前明確落地。
9. dnd-kit 導入後的 keyboard accessibility 是否另開 future UX；目前 4.8a 主要以本機視覺拖曳手感為先。
10. transportation visual attachment 目前採「跟前一張 destination sortable wrapper 移動」；若未來要改成更獨立的 transport card layout animation，應另開 polish，不要影響正式 transport role model。
11. Phase 4.5 untimed slot rebase 是否需要長期改成更明確的 order model，而不是只使用負值 sort_order 編碼，待 Phase 4 後續穩定後評估。
12. Tail transport 在 endpoint 恢復 timed 後的有效性判斷，後續是否需要更明確的 helper 測試或 RPC 層保護。
13. Fixed card 與「接續」disabled 規則目前只限制自動接續，不限制單純儲存；若未來儲存也造成過多混亂，再評估是否加入提示。
14. fixed / untimed 規則已收斂：untimed 不可 fixed；legacy fixed untimed 視為髒資料並於 normalize 清除 fixed 狀態。
15. Phase 4.8b QA 補充 tail_pending + untimed + 新增 timed visit 的 narrow bypass：只有 tail_pending 在本次新增 timed visit 時升級為 tail_promoted_pair，且 blocking untimed 位於 tail_pending 與 promoted target 之間時，才允許最小 rebase；一般 untimed 仍不可因 timed 新增 / 恢復 / 拖曳而自動移動或 compact。
16. 已套用的 production migrations 019～024 不可原地修改；未來 schema / RPC / permission 修正都必須使用 migration 025+。
...
4.8b 建議先做 Demo Timeline Data Parity Polish，讓 /demo/timeline 的 mock data shape、transport role、pair 欄位與 render flow 更接近 Formal，避免本機 UI QA 被 Demo 資料模式誤導；
4.8c 再做 Collaborative Drag Presence，讓多人協作時可看到拖曳中狀態並暫時禁止同日多人同時拖曳，同時加入 heartbeat / timeout / cleanup 防止拖曳 presence 卡死；
4.9 再進 Map 整合前置設計；
4.10 最後做 QA 與交接。
```
