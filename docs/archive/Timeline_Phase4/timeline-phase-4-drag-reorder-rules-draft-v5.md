...
Date: 2026-06-23
Status: Draft for discussion / updated with Collaborative Drag Presence + active/passive untimed rules + untimed slot rebase rules + tail transport restore + fixed auto-continuation guard

> 本文件整理目前討論過的 Timeline Phase 4 後續規則。
> 目前專案已完成至 Phase 4.5；本次補上 active untimed drag、passive untimed conversion、untimed slot rebase / no-compact、tail transport restore，以及 fixed card 與接續限制規則。
...
#### 8. Tail transport restore 規則

若尾端交通卡的 endpoint visit 因被動 untimed conversion 變成 untimed，交通卡可保留並顯示 warning。

當該 endpoint visit 重新恢復為 timed visit 時：

- 若 endpoint visit 仍存在。
- 該 visit 恢復後仍符合尾端位置 / tail transport 關係。
- tail transport 的基本關聯仍成立。

則原本 tail transport 應自動恢復為有效 tail transport，不應進入 invalid transport stack。

不應發生：

- tail transport 因 endpoint 曾經 untimed 而永久失效。
- endpoint 恢復 timed 後，tail transport 被誤判為 invalid。
- tail transport 被自動改成 normal A→B transport。
- tail transport 被自動刪除。

若恢復 timed 後時間不足，只能顯示 transportation shortage warning；這不是 invalid transport。

真正可能進入 warning / invalid / confirm flow 的情境：

- 使用者再次移動 tail transport 上方或下方相關行程。
- endpoint visit 已不存在。
- endpoint 恢復 timed 後已不在尾端，tail 關係真的不成立。
- 使用者明確刪除交通卡。
...
### tail transport restore

若尾端交通卡的 endpoint visit 被動轉成 untimed：

- 交通卡可保留並顯示 warning
- endpoint 恢復 timed 且仍符合尾端關係時，交通卡自動恢復有效
- 不應進入 invalid transport stack
- 不應自動改成 normal transport
- 若恢復後時間不足，只顯示 transportation shortage warning
...
## 9. Phase 4.8：Collaborative Drag Presence
...
4.5 定義未設時間卡可混排行程但不影響時間，主動拖曳不可插入交通 pair，被動轉 untimed 則保留既有交通卡並 warning；同時補上 untimed slot rebase 規則：timed→untimed 可 rebase 保護位置，untimed→timed 不可 compact 其他未設定時間卡；tail transport endpoint 恢復 timed 且仍符合尾端關係時應自動恢復有效；
