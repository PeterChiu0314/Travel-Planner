...
Date: 2026-06-23
Status: Draft for discussion / updated with Collaborative Drag Presence + active/passive untimed rules

> 本文件整理目前討論過的 Timeline Phase 4 後續規則。  
> 目前專案已完成至 Phase 4.5；本次補上 active untimed drag 與 passive untimed conversion 的交通卡規則。  
...
Phase 4 後續應拆開處理：
4.4 先做修改時間後局部接續；
4.5 定義未設時間卡可混排行程但不影響時間，主動拖曳不可插入交通 pair，被動轉 untimed 則保留既有交通卡並 warning；
