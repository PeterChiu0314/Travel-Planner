...
- The alternative editor keeps its map-point controls permanently expanded: the static `地圖點位` heading replaces the former `更改地點` toggle while the Google Map link stays in the header. Its Delete Alternative and Return to Main Itinerary actions use transparent secondary surfaces.
...
- Expanded visit notes preserve their wrapping and use 13 px / 400 text. Expanded visit and transportation cards share the same responsive `Wallet` budget row: the 14 px / 400 label reads `預算`, and the divider plus content-width pale-green budget tags stay beside it when space permits, wrapping together only when constrained. Transportation places this row below its note with the visit-card spacing rhythm.
- Existing alternatives render as one unframed, non-interactive flex row: a 16 px Lucide `Files` icon plus 14 px / 400 `備案` (or `原行程` on the alternative face), followed by a thin divider and 400-weight plain `類型・目的地` summary that wrap together when space is constrained. The row aligns with the details divider, has no chevron or delete action, uses an 8 px top gap, and keeps the lower-right flip control unchanged. Expanded note copy is 400 weight; the compact lower-left Google Map link keeps 12 px top spacing with `2px 6px` padding and a `-6px` left margin.
...
- The visit Type field now uses the same custom menu surface and Chevron treatment as the time controls instead of the browser-native select. Time segments use 500 weight with no gap, left alignment, and 4 px container padding; all menu toggles are 24 px, and Type/Start/End/Duration menus share the Dayboard 4 px scrollbar treatment. In-app Browser QA confirmed selection updates, zero horizontal overflow, shared scrollbar styles, and zero console errors; `npm.cmd run build` and `git diff --check` passed.
- Final compact-control polish sets Type-menu selected/hover weight to 600, location action weight to 500, removes the Type arrow divider, uses 22 px Hour/Minute segments with 6 px group padding, and gives Duration 9 px left padding. The focused Phase 5.9 editor smoke test and `git diff --check` passed.
...
- The no-frame linked-budget/alternative refinement passed the full `mapProviderPrep` + `phase-1-7f-smoke` regression set 73/73 and the production build. Focused coverage confirms the 500-weight Wallet heading, content-width budget tag, same-row placement with automatic constrained-width wrapping, 14 px alternative label, 16 px icon, thin separators, and borderless summary.
- The follow-up compact-width refinement verifies 400-weight note/alternative copy, 8 px budget and alternative top gaps, grouped alternative wrapping, and the exact expanded Map-link inset/padding values.
