...
```text
Timeline Phase 5.7c-1 Node-level Collaborative Route Editing - Complete
```

Next phase:

```text
Phase 5.7d has been merged into Phase 5.7c. Phase 5.7c keeps the same-day itinerary editable, has no 15-second idle exit, and treats custom route lines as secondary data that yields to itinerary changes. Collaborative route editing uses one route node as the minimum synchronization and persistence unit. Presence is low-frequency editor state with a 32-second heartbeat and user-deduplicated labels; node locks and add/move/delete previews use Broadcast with 120 ms latest-wins drag moves. PostgreSQL persists every custom node independently in `itinerary_route_override_nodes`, while `itinerary_route_overrides` remains the segment container. Channel health is checked silently on heartbeat, focus, online, and visibility recovery; replacement channels reload authoritative nodes before editing resumes. Final stabilization preserves remote previews across handoff, releases stale local ownership, stores pending commits per `segmentKey + nodeId`, persists preview-only deletes, restores authoritative nodes after failed DELETE, and preserves batched ownership-start events separately from position finals. Deployed Chrome QA covers add/delete, same-node and different-node handoff, P1-to-P2 rapid drag, five-node concurrency, editor deduplication, 319-second background recovery, endpoint/reorder/destination-delete invalidation, failure rollback, normal-delete regression, unaffected-adjacency retention, and refresh convergence. Phase 5.7c closeout is complete. The damaged historical plan in `docs/todo/2026-07-09-phase-5-7c-collaborative-route-edit-plan.md` is not authoritative; use the 2026-07-11 handoff/closeout.
```
...
- Reversible destination-delete QA created `TEMP_ROUTE_DELETE_QA` with a valid map coordinate and one adjacent route node while retaining two nodes on an unaffected segment. Deleting the temporary destination removed it on both clients, reduced B from three to two nodes without refresh, preserved the same two unaffected node positions on A/B, and remained identical after both refreshed. The temporary destination and all QA route nodes were then removed; final refresh confirmed zero temporary items and zero custom nodes on both clients.
- User dual-account testing and the complete deployed Chrome matrix confirm Phase 5.7c multiplayer route editing is stable enough to close.
...
Chrome reversible destination-delete QA created TEMP_ROUTE_DELETE_QA and an adjacent node alongside two unaffected nodes. Deleting the temporary destination removed it on both clients and reduced the active remote editor from three nodes to two without refresh; both clients showed the same two retained positions before and after refresh. Cleanup removed the temporary item and all QA nodes, and final refresh showed zero/zero
latest pushed implementation commit: c6989a3 Preserve rapid route ownership handoff
```
...
- Reorder, endpoint-coordinate, and reversible destination-delete invalidation passed deployed two-client QA, including unchanged-adjacency retention and refresh convergence.
...
Phase 5.7c-1 is complete at `c6989a3 Preserve rapid route ownership handoff` on `codex/timeline-phase-5-7`; closeout evidence is in `docs/2026-07-11-phase-5-7c-node-collaboration-handoff.md`. Deployed two-client Chrome QA passed remote delete, drag-then-delete, final-node delete, preview-only delete persistence, failed-delete rollback, normal-delete regression, concurrent add to the five-node limit, simultaneous different-node drag, A-to-B-to-A same-node handoff, P1 handoff immediately followed by P2, same-account editor deduplication, 319-second background recovery with a synchronized first drag, refresh convergence, endpoint-coordinate invalidation, reorder invalidation, destination-delete invalidation, and unchanged-adjacency retention. Production migration `20260712033758` is applied; both route tables use full replica identity and belong to `supabase_realtime`. No Phase 5.7c stabilization work remains. Preserve the closeout architecture: do not restore same-day itinerary locking, a 15-second idle exit, segment-snapshot Broadcast, whole-`points_json` multiplayer writes, Routes/Directions travel-time queries, Google route polylines, or unrelated Timeline/Places/Auth/Budget changes. Choose the next product phase explicitly before new implementation.
