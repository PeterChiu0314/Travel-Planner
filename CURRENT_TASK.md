...
- Existing alternatives render as one unframed, non-interactive row: a 16 px Lucide `Files` icon plus 14 px `備案` (or `原行程` on the alternative face), a thin divider, and the plain `類型・目的地` summary. The row aligns with the details divider, has no chevron or delete action, and keeps the lower-right flip control unchanged. The alternative row and the compact lower-left Google Map link each use 12 px top spacing.
