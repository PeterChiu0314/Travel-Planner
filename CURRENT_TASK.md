...
- The visible product name is `旅程工房` with the English subtitle `Travel Studio`; the browser tab reads `旅程工房｜Travel Studio`. The trip count now sits beneath the smaller `我的旅程` heading instead of beneath the brand.
- Day tabs use the primary color for hover and keyboard focus; the active tab copies the formal preview treatment with a thin primary outline, an 8% primary-tinted surface, and a 2 px inset bottom accent. Sidebar navigation uses a subtle primary-tinted hover/focus surface without adding a border, and trip cards use a primary border with a lightly tinted surface.
