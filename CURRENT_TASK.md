...
- Visit editor time links now use fixed 12 px grid tracks and sit 4 px below the field center (3 px below the preceding position). Each dropdown toggle reserves 42 px from its divider to the field edge, while the segmented-time colon receives a 3 px upward optical correction.
...
- Chrome measured both time links at 12 px wide and +4 px from field center, each dropdown toggle at 42 px, the colon at +0.05 px from field center, zero time-row overflow, and zero console errors. The focused Phase 5.9 editor smoke test and production build passed afterward.
