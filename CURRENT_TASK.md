...
Chrome reproduced an itinerary-priority bug: endpoint coordinate invalidation deleted the authoritative DB override, but stale remote previews kept five handles visible until refresh. The provider now fences this present-to-absent authoritative segment transition in deployed commit 40342b3. Automated verification passes; a fresh deployed endpoint-invalidation replay remains pending because all available test route nodes were consumed by the invalidation QA and Chrome automation could not trigger the first transparent hit-line node add
latest pushed code commit: 40342b3 Clear stale previews after route invalidation
```
