...
- Timeline visit-to-visit, visit-to-transport, and transport-to-visit spacing is consistently 4 px. Empty transport insertion zones keep a 4 px resting layout but extend their transparent hit target 4 px into both adjacent cards, producing a practical 12 px trigger band without changing the compact gap. Hover/focus expands the gap and control to the same centered 22 px height, restores a full-card-width translucent deep-gray ghost surface without border or shadow, aligns the 13 px Lucide Plus and 500-weight label with the visit time column, and places the trailing line 1 px below the geometric center for optical alignment.
...
- Transportation-card computed styles confirmed a 1 px border, 4 px vertical padding, 13 px title, and an 18 px Lucide category icon. The transport-insert zone now preserves a 4 px resting card gap, triggers from 3 px inside both adjacent card edges, and expands to a centered 22 px gap. Its full-width ghost surface uses `rgba(36, 43, 38, .16)` without border or shadow, the 13 px Plus begins within 4 px of the visit time text, and the trailing-line center sits 1 px below the gap center.
